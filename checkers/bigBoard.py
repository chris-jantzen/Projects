...
def piece(row, col, piece=None, type=None):
    ...
    if piece is None:
        square[row][col].create_oval(10, 10, 90, 90, fill='black')
    elif type is 'king':
        square[row][col].create_oval(10, 10, 90, 90, fill=color)
        square[row][col].create_oval(30, 30, 70, 70, width=10, outline='black')
    else:
        square[row][col].create_oval(10, 10, 90, 90, fill=color)
...
def outlineSquare(row, col, value=1):
    if value == 1:
        ...
        color = 'black'
    square[row][col].create_rectangle(7, 7, 96, 96, width=4, outline=color)


def validMoves(row, col, player, jumpOnly=None):
    if jumpOnly == None:
        jumpOnly = False
    global board
    moves = []
...
for row in range(8):
    for col in range(8):
        square[row].append(Canvas(frame2, height=100, width=100, \
